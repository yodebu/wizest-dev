package wizest.media;

/*
 * @(#)SampleDeMux.java	1.3 01/03/13
 *
...
/**
 * GSM 8000 samples per sec. 160 samples represent 20 milliseconds and GSM
 * represents them in 33 bytes. So frameSize is 33 bytes and there are 50 frames
...
public class SampleDeMux implements Demultiplexer {
	...
	private int maxFrame = Integer.MAX_VALUE; // If we don't know the media
												// length
	...
	/**
	 * A Demultiplexer may support pull only or push only or both pull and push
	 * streams. Some Demultiplexer may have other requirements. For e.g a
	 * quicktime Demultiplexer imposes an additional requirement that
	...
	public Track[] getTracks() throws IOException, BadHeaderException {
		...
		/* enabled= */true, new Time(0), numBuffers, bufferSize, minLocation, maxLocation);
		return tracks;
	}
	...
	private void /* for now void */readHeader() throws IOException, BadHeaderException {
		...
		format = new AudioFormat(AudioFormat.GSM, 8000, // sampleRate,
				16, // sampleSizeInBits,
				1, // channels,
				bigEndian ? AudioFormat.BIG_ENDIAN : AudioFormat.LITTLE_ENDIAN, signed ? AudioFormat.SIGNED : AudioFormat.UNSIGNED, (blockSize * 8), // frameSizeInBits
	...
	public int readBytes(PullSourceStream pss, byte[] array, int offset, int numBytes) throws IOException {
		...
		} else if ((offset < 0) || (offset > array.length) || (numBytes < 0) || ((offset + numBytes) > array.length) || ((offset + numBytes) < 0)) {
		...
		while (remainingLength > 0) {
			...
			if (actualRead == -1) {// End of stream
				if (offset == 0) {
					throw new IOException("SampleDeMux: readBytes(): Reached end of stream while trying to read " + numBytes + " bytes");
				} else {
					return offset;
				}
				...
				throw new IOException("SampleDeMux: readBytes() read returned " + actualRead);
			}
	...
	abstract private class BasicTrack implements Track {
		...
		private long mediaSizeAtEOM = -1L; // update when EOM implied by
											// IOException occurs

		BasicTrack(SampleDeMux parser, Format format, boolean enabled, Time duration, Time startTime, int numBuffers, int dataSize, PullSourceStream stream) {
			this(parser, format, enabled, duration, startTime, numBuffers, dataSize, stream, 0L, Long.MAX_VALUE);
		}

		/**
		 * Note to implementors who want to use this class. If the maxLocation
		 * is not known, then specify Long.MAX_VALUE for this parameter
		 */
		public BasicTrack(SampleDeMux parser, Format format, boolean enabled, Time duration, Time startTime, int numBuffers, int dataSize, PullSourceStream stream, long minLocation, long maxLocation) {
			this.parser = parser;
			...
			try {
				...
				if (maxLocation != Long.MAX_VALUE) {
					...
					if (length != SourceStream.LENGTH_UNKNOWN) {
						...
						// The next readFrame will read the remaining data till
						// EOM.
						maxLocation = length;
						maxStartLocation = maxLocation - dataSize;
		...
		public long getMediaSizeAtEOM() {
			return mediaSizeAtEOM; // updated when EOM implied by IOException
									// occurs
		}
	}
	...
	private class GsmTrack extends BasicTrack {
		...
		GsmTrack(AudioFormat format, boolean enabled, Time startTime, int numBuffers, int bufferSize, long minLocation, long maxLocation) {
			super(SampleDeMux.this, format, enabled, SampleDeMux.this.duration, startTime, numBuffers, bufferSize, SampleDeMux.this.stream, minLocation, maxLocation);
		...
		public int mapTimeToFrame(Time t) {
			...
			System.out.println("mapTimeToFrame: " + (int) time + " ==> " + frameNumber + " ( " + frameNumber + " )");
			return frameNumber;
		}
		...
		public Time mapFrameToTime(int frameNumber) {
			...
			System.out.println("mapFrameToTime: " + frameNumber + " ==> " + time);
