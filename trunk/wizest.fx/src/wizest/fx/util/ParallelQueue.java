...
/**
 * 하나의 입력과 하나의 출력을 가지지만 내부적으로 여러개의 대기열을 가진 queue이다. ParallerQuenuEntry의 Line Id에
 * 따라서 여러줄의 대기열을 만들고 각 줄은 '평등한 관계'로 '같은 비율'로 꺼내지게 된다. 즉, 같은 line id를 가질 경우 선착순
 * 처리를 하지만 다른 line id들 끼리는 선후 관계 없이 같은 비율로 처리된다. 각 줄의 운용은 line id가 새롭게 입력될 경우
 * 새로운 줄이 생성되며 하나의 줄의 entry가 비게 되면 전체 줄에서 제거된다.
 * 
 */
...
public class ParallelQueue {
	...
	/**
	 * entry의 line id를 참고 하여 적당한 줄 뒤에 집어넣는다. 기존 line id가 없을 경우 새로운 line을 생성한다.
	 * 
	 * @param entry
	 */
		...
		for (int i = 0; i < lines.size(); ++i) {
			...
			for (int j = 0; j < line.size(); ++j) {
				if (tc.isTarget(line.get(j))) {
					ParallelQueueEntry entry = (ParallelQueueEntry) line
							.remove(j);
					if (line.isEmpty())
						lines.remove(i);
					return entry;
		...
		for (int i = 0; i < lines.size(); ++i) {
			...
			for (int j = 0; j < line.size(); ++j) {
				if (tc.isTarget(line.get(j))) {
					...
					ParallelQueueEntry entry = (ParallelQueueEntry) line
							.remove(j--); // 지울때 왼쪽으로 쉬프팅 되므로 index를 하나 줄임
					if (line.isEmpty()) {
