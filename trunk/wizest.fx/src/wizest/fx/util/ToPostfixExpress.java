...
public class ToPostfixExpress {
	...
	private HashMap opsPriorityMap;
	private HashMap opsOperandNumber;

	private GenericCache cache;
	private static final int MAX_CACHE = 100;

	/**
	 * 첫번째 인덱스 : 연산에 필요한 피연산자수 -> 단항연산자, 이항연산자,... index번호가 피연산자수를 의미한다. 두번째 인덱스
	 * : 우선순위 세번째 인덱스 : 동일한 우선순위의 연산자 배열
	 * 
	 * <code>
	 *  연산자 예제)
	 *  String[][][] ops = {
	...
	public ToPostfixExpress(String[][][] orderedOperators) {
		...
		this.opsPriorityMap = new HashMap();
		this.opsOperandNumber = new HashMap();
		...
		plainOps = (String[]) ol.toArray(new String[0]);
		// 길이가 긴것을 앞으로해서 정렬
		Arrays.sort(plainOps, new Comparator() {
	...
	public PostfixExpress toPostfixExpress(String evalString) {
		...
		ArrayList ex = new ArrayList(); // expression
		ArrayList no = new ArrayList(); // 필요한 피연산자 수 -> 0 보다 클경우 연산자다. 단항연산자 1,
										// 이항연산자 2, (아직 단항 연산자는 지원하지 않지만 ㅡ.ㅡ")
		...
		if (parenthesisChecker != 0) {
			...
			for (int i = 0; i < nos.length; ++i) {
				nos[i] = ((Integer) no.get(i)).intValue();
			}
	...
	private int compareOpsPriority(String op1, String op2) {
		int p1 = ((Integer) opsPriorityMap.get(op1)).intValue();
		int p2 = ((Integer) opsPriorityMap.get(op2)).intValue();

		return p2 - p1;
	}

	public static boolean checkVectorOperationSyntax(PostfixExpress ex,
			String[] vectorOps) {
		// 집합의 연산이 적절한가
	...
	// {
	// String eval =
	// " ldsklsdg> !\\\\\\(sdljsdkgs\\) && fsdalajfsag < !(gaklgaj ==dsagsaogdaag) =s ";
	//
	// ToPostfixExpress toPostfix = new ToPostfixExpress(new String[][][] {
	// // operand 0
	// {{}},
	// // operand 1
	// {{"(",")"},{"!"}},
	...
	// );
	//
	// System.out.println( toPostfix.toPostfixExpress(eval));
	// }
