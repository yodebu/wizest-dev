...
public class HttpUtils {
	...
	/**
	 * HttpServletRequest.getQueryString()는 이것을 실행하기 전에 request.getParameter()
	 * 같은 것이 불려 지지 않으면 request character encoding을 하지 않은 값을 리턴한다. 또한 request
	 * character encoding가 지정 되어 있지 않으면 ISO-8859-1로 리턴하는 문제점이 있다. 이 메소드는
	 * request의 character encoding과 locale을 참고로 encoding한 query string을 돌려 주며
	 * 이전에 request.getParameter() 와 같은 메소드가 불려지는 것과 관계 없이 항상 동일하게 지역화 된 문자열을
	 * 리턴하게 한다.
	 * 
	 * @param request
	 * @param doURLDecoding
	 *            query string을 URL decoding을 할까요?
	 * @return encoding한 query string을 돌려준다.
	 */
	// public static String getLocalizedQueryString(HttpServletRequest
	// request,boolean doURLDecoding)
	// {
	 ...
	 * .apache.struts.Globals.LOCALE_KEY,java.util.Locale.KOREA);%>
	 * 
	 * @param locale
	 *            HTTP response 에 지정할 locale, 이것에 따라 웹브라우저의 character set이 결정된다.
	 * @param pageContext
	 */
	// public static void setHttpResponseLocale(Locale locale,PageContext
	// pageContext)
	// {
	 ...
	 *            HTTP response 에 지정할 locale, 이것에 따라 웹브라우저의 character set이 결정된다.
	 * @param session
	 *            struts의 i18n지원을 위해 지정하고자 하는 locale을 설정을 session에
	 *            org.apache.struts.Globals.LOCALE_KEY 의 key로 등록한다.
	 * @param response
	 */
	private static String strutsLocaleKey = null;
	static {
		// 컴파일시에 struts library가 필요하지 않도록
		// session.setAttribute(org.apache.struts.Globals.LOCALE_KEY,locale) 를
		// 돌려서 표현
		try {
			Class org_apache_struts_Globals = Class
					.forName("org.apache.struts.Globals");
			// strutsLocaleKey=(String)org_apache_struts_Globals.getDeclaredField("LOCALE_KEY").get(new
			// Object());
			strutsLocaleKey = (String) org_apache_struts_Globals
					.getDeclaredField("LOCALE_KEY").get(
							org_apache_struts_Globals.newInstance());
		} catch (Throwable t) {
			strutsLocaleKey = null;
		}
	}

	// public static void setHttpResponseLocale(Locale locale,HttpSession
	// session,ServletResponse response)
	// {
	...
	// public static void setHttpResponseCharset(ServletResponse response,
	// String charset)
	// {
	// if (charset==null)
	// response.setContentType("text/html");
	...
	/* 어드민 메뉴때문에 만든 메소드 */
	public static String alterString(boolean chkvalue, String yesString,
			String noString) {
		if (chkvalue) {
			return yesString;
		} else {
	...
	public static String replaceBlock(String startBlock, String endBlock,
			String value, String startReplaceStr, String endReplaceStr) {
		if (value == null) {
			return (null);
		}
		...
		while (true) {
			...
			strReturn += value.substring(beginIndex + startBlock.length(),
					endIndex);
			strReturn += endReplaceStr;
			strReturn += value.substring(endIndex + endBlock.length(), value
					.length());
			beginIndex = beginIndex + startReplaceStr.length()
					+ (endIndex - (beginIndex + startBlock.length()))
					+ endReplaceStr.length();
			value = strReturn;
		}
		return value;
	}

	public static String replaceBlockAll(String startBlock, String endBlock,
			String value, String replaceStr) {
		if (value == null) {
			return (null);
		}
	...
	public static String abstractFirstBlock(String startBlock, String endBlock,
			String value, String startReplaceStr, String endReplaceStr) {
		if (value == null) {
			return (null);
		}
	...
	public static String selectDeleteString(String value, String startBlock,
			String endBlock) {
		if (value == null) {
			return (null);
		}
		...
		if (spos != -1) {
			...
			tmpString += value.substring(value.indexOf(endBlock, spos)
					+ endBlock.length());
			return tmpString;
		} else {
			return value;
	...
	public static String filterWML(String text) {
		...
		text = text.replaceAll("</[p|P]>", START_DELIM + "br/" + END_DELIM);
