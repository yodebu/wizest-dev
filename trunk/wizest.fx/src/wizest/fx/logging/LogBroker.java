...
public class LogBroker {
	...
	// 적절한 pattern matching을 위해 정렬이 필요하다.(순서대로 비교) 정렬 방법은 PatternComparator 을
	// 따른다.
	private TreeMap patternMapping = null;
	...
	/*
	 * <logs> <handler> <description>File handler.</description>
	 * <handler-name>fileHandler</handler-name> <configured-object>
	 * <class-name>java.util.logging.FileHandler</class-name> <constructor>
	 * <constructor-param> <description>pattern</description>
	 * <type>java.lang.String</type> <value>%h/java%u.log</value>
	 * </constructor-param> <constructor-param> <description>limit</description>
	 * <type>int</type> <value>50000</value> </constructor-param>
	 * <constructor-param> <description>count</description> <type>int</type>
	 * <value>1</value> </constructor-param> <constructor-param>
	 * <description>append</description> <type>boolean</type>
	 * <value>true</value> </constructor-param> </constructor>
	 * </configured-object> <level>INFO</level>
	 * <formatter>java.util.logging.XMLFormatter</formatter>
	 * <encoding>euc-kr</encoding> </handler> <handler> <description>Console
	 * handler.</description> <handler-name>consoleHandler</handler-name>
	 * <configured-object>
	 * <class-name>java.util.logging.ConsoleHandler</class-name>
	 * </configured-object> <level>INFO</level>
	 * <formatter>java.util.logging.SimpleFormatter</formatter>
	 * <encoding>euc-kr</encoding> </handler> <logger> <description>Framework
	 * default logger configuration.</description>
	 * <logger-name>frameworkLogger</logger-name>
	 * <handler-name>fileHandler</handler-name>
	 * <handler-name>consoleHandler</handler-name> </logger> <logger>
	 * <description>Default logger configuration.</description>
	 * <logger-name>defaultLogger</logger-name>
	 * <handler-name>consoleHandler</handler-name> </logger> <logger-mapping>
	 * <description>Mapping for framework classes</description>
	 * <pattern>wizest.fx.</pattern> <logger-name>frameworkLogger</logger-name>
	 * </logger-mapping> <logger-mapping> <description>Default
	 * mapping.</description> <pattern></pattern>
	 * <logger-name>defaultLogger</logger-name> </logger-mapping> </logs>
	 */
	 ...
	 * </pre>
	 * 
	 * &lt;configured-object/&gt; 부분은 wizest.fx.util.InstanceCreator을
	 * 따른다.
	 * 
	 * @param config
	 * @throws InitializingException
	 * 
	 * @see wizest.fx.util.InstanceCreator
	 */
	private LogBroker(Configuration config) throws InvalidSetupException {
		this();
			...
			try {
				...
				for (int i = 0; i < loggerConfigs.length; ++i) {
					...
					for (int j = 0; j < handlerNameConfigs.length; ++j) {
						...
						if (handler == null)
							throw new InvalidSetupException("not found handler:" + handlerName);
	...
	private String resolve(String name) {
		...
		if (loggerName == null) {
			// pattern에서 name을 찾는다.
			String[] patterns = (String[]) patternMapping.keySet().toArray(new String[0]);
			for (int i = 0; i < patterns.length; ++i) {
				if (isCorrespondingClass(patterns[i], name)) {
					loggerName = (String) patternMapping.get(patterns[i]);
	 ...
	 * 새로운 mapping을 추가한다.
	 * 
	 * keyName loggerName a.b.c.* -> logger1 a.b.c.A -> logger2 a.b.c.d.* ->
	 * logger3
	 * 
	 * @param keyName
	 *            pattern 또는 name key
	...
	// public static void addLoggerMapping(String map, Logger log) {
	// LogBroker lb = LogBroker.getInstance();
	// lb.addMapping(map, log.getName());
	// }

	// private static LogBroker getInstance() {
	// if (instance == null)
	// instance = new LogBroker();
	// return instance;
	// }

	public static LogBroker initialize(Configuration config) throws InvalidSetupException {
		if (instance == null)
			instance = new LogBroker(config);
		return instance;
	}
	...
	public static Logger getLogger(String name) {
		...
		Logger l;
		if (instance != null)
			l = Logger.getLogger(instance.resolve(name));
		else
	...
	/**
	 * object의 class name 명을 name으로 하는 logger를 가져온다. (getLogger(String)의
	 * wrapper)
	 * 
	 * @param obj
	 * @return
