 ...
 * @author wizest
 */
public class HtmlTagNameParser extends AbstractParser implements TagNameParser {
    ...
    public Marker parseNext() throws ParserException {
        ...
        if (getParsingPoint() < HtmlTagParser.TAG_OPEN.length()) // name 은 tag당 하나 밖에 없으니깐 한번만 pasing한다. ㅡ.ㅡ;;
        {
            start = HtmlTagParser.TAG_OPEN.length();
            end = text.indexOf(' ');
            if (text.charAt(start) == '/') {
                ...
                // 속성이 없다면
                if (end < 0 || end == start)
                    end = text.length() - HtmlTagParser.TAG_CLOSE.length();
            } else {
                isStartTag = true;
                if (text.charAt(text.length() - HtmlTagParser.TAG_CLOSE.length() - 1) == '/')
                    isEndTag = true;
                else
                    isEndTag = false;
                // 속성이 없다면
                if (end < 0 || end == start)
                    end = text.length() - HtmlTagParser.TAG_CLOSE.length() + (isEndTag ? -1 : 0);
            }
            if (end > start) {
