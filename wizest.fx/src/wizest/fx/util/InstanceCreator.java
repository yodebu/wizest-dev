...
/**
 * Java Introspection & Reflection 기능을 이용할 때 Class 와 Object를 얻기 쉽게한다. String으로 된
 * Class 이름과 Value를 주면 해당 Class 또는 Object로 Instance화 시킨다.
 * 
 */
public class InstanceCreator {
	 ...
	 * </pre>
	 * 
	 * &lt;class-name/&gt; 는 반드시 필요하며 &lt;constructor/&gt; 와 &lt;property/&gt; 는
	 * 없어도 된다. <br>
	 * <br>
	 * * inner configured object를 허용한다. -><b>&lt;value/&gt; 안에
	 * &lt;configured-object/&gt; 가 들어가도 괜찮다.!! </b> <br>
	 ...
	 */
	public static Object createConfiguredInstance(Configuration config)
			throws InstanceCreationException {
		/*
		 * <configured-object> <class-name>java.util.logging.FileHandler
		 * </class-name> <constructor> <constructor-param> <description>pattern
		 * </description> <type>java.lang.String </type> <value>%h/java%u.log
		 * </value> </constructor-param> <constructor-param> <description>limit
		 * </description> <type>int </type> <value>50000 </value>
		 * </constructor-param> <constructor-param> <description>count
		 * </description> <type>int </type> <value>1 </value>
		 * </constructor-param> <constructor-param> <description>append
		 * </description> <type>boolean </type> <value>true </value>
		 * </constructor-param> </constructor> <property> <name> </name> <value>
		 * </value> </property> </configured-object>
		 */
		if (!config.getTagName().equals("configured-object")) {
			throw new InstanceCreationException(
					"invalid configuration. only <configured-object/> allowed.");
		}
		try {
			// 생성자 파라미터가 존재할 때
			Object object = null;
			...
			// construct
			try {
				Configuration[] constructorParams = config
						.getChildren("constructor.constructor-param");
				...
				for (int j = 0; j < constructorParams.length; ++j) {
					String paramType = constructorParams[j].getChild("type")
							.getValue();
					Class paramClass = getClassInstance(paramType);
					Object paramObject = null;
					try {
						// value가 다시 <configured-object/> 로 구성된 object 일 경우 재귀
						// 호출!!
						paramObject = InstanceCreator
								.createConfiguredInstance(constructorParams[j]
										.getChild("value.configured-object"));
						...
						try {
							...
							// value가 <null/> 가 아닐 경우
							String paramValue = constructorParams[j].getChild(
									"value").getValue();
							paramObject = getObjectInstance(paramClass,
									paramValue);
						}
					}
				...
				object = objectClass.getConstructor(paramClasses).newInstance(
						paramObjects);
			...
			try {
				...
				for (int i = 0; i < properties.length; ++i) {
					...
					if (setter == null) {
						throw new InstanceCreationException(
								"could not find setter method of property name:"
										+ name);
					}
					if (setter.getParameterTypes().length != 1) {
						throw new InstanceCreationException(
								"unknown parameter for property setter:propery name="
										+ name);
					}
					Object paramObject = null;
					try {
						...
						// 호출!!
						paramObject = InstanceCreator
								.createConfiguredInstance(property
										.getChild("value.configured-object"));
					} catch (NotFoundXmlTagException nx) {
						try {
							// value가 다시 <null/> 일 경우 null
							...
							// value가 <null/> 가 아닐 경우
							String value = property.getChild("value")
									.getValue();
							paramObject = getObjectInstance(setter
			...
			throw new InstanceCreationException(
					"Occurred configuration exception.", cx);
		} catch (ClassNotFoundException cnfe) {
			throw new InstanceCreationException("Class not found.", cnfe);
		} catch (Exception e) {
			throw new InstanceCreationException(
					"Could not instantiate the object.", e);
		}
	}
	 ...
	 * @throws ClassNotFoundException
	 */
	public static Class getClassInstance(String className, ClassLoader loader)
			throws ClassNotFoundException {
	...
	/**
	 * clazz를 value로 생성한 새로운 object를 리턴 clazz가 primitive type일 경우 해당하는 wapper
	 * class를 생성해서 돌려준다.
	 * 
	 ...
	 * 
	 * @see wizest.fx.logging.LogBroker
	 */
	public static Object getObjectInstance(Class clazz, String value)
			throws InstanceCreationException {
		Object object = null;
		if (clazz.isPrimitive()) {
			if (clazz == int.class) {
				...
				} catch (NumberFormatException nf) { // value가 숫자형이 아니어서 에러가 생기면
														// 0으로
					object = new Integer(0);
				}
			} else if (clazz == boolean.class) {
				// 원래 spec에 따르면 "yes"는 boolean으로 false이지만 "yes"일 경우에도 true로 한다.
				// 그외에도 T,Y도 true로 한다.
	 ...
	 * @return
	 */
	public static Object getObjectInstance(String className, String value)
			throws ClassNotFoundException, InstanceCreationException {
	 ...
	 * @throws InstanceCreationException
	 */
	private static HashMap getWriteMethodMapTable(Class clazz)
			throws InstanceCreationException {
		try {
			// property 의 set method 를 table화 시킴
			HashMap methodMap = (HashMap) cachedMethodMapTable.get(clazz);
			if (methodMap == null) {
	...
	private static Hashtable cachedMethodMapTable = new Hashtable(); // write
																		// method
																		// 를
																		// cache해서
																		// introspection을
	 ...
	 * @throws InstanceCreationException
	 */
	public static Object createBeanWithPropertyValues(Class beanClass,
	 ...
	 * 
	 * 주의1: 같은 property에 해당하는 names와 values는 index가 서로 일치해야한다. 주의2: property
	 * setter method 찾지 못할 경우 아무런 메세지 없이 skip한다.
	 * 
	 * @param beanObject
	 * @param propertyNames
	 * @param propertyValues
	 ...
	 * @throws InstanceCreationException
	 */
	public static Object setPropertyValues(Object beanObject,
			String[] propertyNames, Object[] propertyValues)
			throws InstanceCreationException {
		try {
			// property 의 set method 를 table화 시킴
			Class beanClass = beanObject.getClass();
			HashMap methodMap = getWriteMethodMapTable(beanClass);
			String[] names = propertyNames;
			for (int i = 0; i < names.length; ++i) {
				String name = names[i]; // .toLowerCase(); 주의!!!!!!!!!!!!!!
				Object value = propertyValues[i];
				Method setter = (Method) methodMap.get(name);
				// Exception 대신 warning 으로 대체!
				// if( setter == null )
				// throw new InstanceCreationException(
				// "could not find setter method of property name:" + name );
				if (setter == null) {
					// System.err.println("could not find setter method of property name:"
					// + name);
					continue;
				}
				// try
				// {
				// value = adjustMethodParameterType( setter, value );
				// setter.invoke( beanObject, new Object[] {value} );
				// }
				// catch( InstanceCreationException ex )
				// {
				// System.err.println("InstanceCreator.setPropertyValues():CAUTION:"+ex.getMessage()+":setting passed.");
				// }
				// 여기까지 아래 블럭으로 교체
				try {
					setter.invoke(beanObject, new Object[] { value });
				} catch (Exception ex) { // 여기 부분을 최적화 할 필요가 있음.
					if (value == null) { // primitive 타입에 null을 집어 넣었을 경우 여기로
											// 온다. (object형에는 null을 줄 수 있으니
											// exception이 발생하지 않는다.)
						// throw new
						// InstanceCreationException("primitive type do not allow null.");
						System.err
								.println("InstanceCreator.setPropertyValues():CAUTION:primitive type do not allow null.:setting passed.");
					} else { // setter의 parameter 타입이 맞지 않을 경우 - type mismatch
						Class param = setter.getParameterTypes()[0];
						if (param.isPrimitive()) {
							// setter의 argument가 primitive형이면서 value가 Number형이면
							// 형을 맞게 변형시켜 재시도!
	...
	// /**
	// * method parameter type와 object의 type을 체크한 후 object 형을 변형해야할 경우 변형하여
	// 리턴한다.
	...
	// * @throws InstanceCreationException
	// */
	// private static Object adjustMethodParameterType(Method m,Object o) throws
