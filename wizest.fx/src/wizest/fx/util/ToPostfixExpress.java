...
public class ToPostfixExpress
{
    ...
    /**
     * 첫번째 인덱스 : 연산에 필요한 피연산자수 -> 단항연산자, 이항연산자,... index번호가 피연산자수를 의미한다.
     * 두번째 인덱스 : 우선순위
     * 세번째 인덱스 : 동일한 우선순위의 연산자 배열
     *
     * <code>
     *  연산자 예제)
     *  String[][][] ops = {
    ...
    public PostfixExpress toPostfixExpress(String evalString)
    {
        ...
        ArrayList no=new ArrayList(); // 필요한 피연산자 수 -> 0 보다 클경우 연산자다.   단항연산자 1, 이항연산자 2, (아직 단항 연산자는 지원하지 않지만 ㅡ.ㅡ")
...
//    {
//        String eval = " ldsklsdg> !\\\\\\(sdljsdkgs\\) && fsdalajfsag < !(gaklgaj ==dsagsaogdaag) =s ";
//
    ...
    private static String filterSlashMark(String s)
    {
        if(USE_SLASH_MARK) {
            ...
            for(int i=0,length=s.length();i < length;++i) {
                if(s.charAt(i) == SLASH_MARK)++slashCnt;
                else slashCnt=0;
