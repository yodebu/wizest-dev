...
/**
 * BEGIN 이 [[ 이고 END 가 ]] 이면 다음과 같이 parsing 된다. (trimming됨) abcdefg[[ hijklm
 * ]]nopqrstuvwxyz -> hijklm
 */
...
public class GenericParser extends AbstractParser {
	 ...
	 *            String
	 * @param overlap
	 *            boolean true일 경우 END를 다음 parsing떄 parsing의 시작지점에 포함시킨다.
	 */
	public GenericParser(String BEGIN, String END, boolean overlap,
			boolean showTag) {
		this.BEGIN = BEGIN;
		this.END = END;
		...
		if (overlap && (beginLen == 0 || endLen == 0))
			throw new IllegalArgumentException(
					"not allow 0 length mark with peel off mode");
	...
	public Marker parseNext() throws ParserException {
		...
		String t = (showTag) ? text.substring(b, e) : text.substring(
				p1 + beginLen, p2).trim();
