...
import wizest.fx.session.SessionManagerFactory;
import wizest.fx.session.SimpleSessionManagerFactory;


/**
 * time-out object pool
 * (일정 시간동안 가져다 쓰지 않으면 pool의 object가 없어진다.)
 */
...
//    private int inactiveIntervalSecond = 60;
    ...
    /**
     * @param inactiveIntervalSecond 억세스 하지 않을 경우 object를 pool에서 삭제해 버릴 time-out(초) 시간
     * @param maxObject pool에 수용할 최대 object 수
     */
...
//        this.inactiveIntervalSecond = inactiveIntervalSecond;
    ...
    /**
     * pool에서 object class name 에 해당하는 object 를 가져온다.
     * 여유분이 없을 경우 null;
     *
     * @param tagName
     * @return
...
//        s.removeAttribute(key);
    ...
    /**
     * pool에 object를 className에 맞게 추가한다.
     * pool에 존재하는 object 수가 maxObject 보다 많으면 무시한다.
     *
     * @param tagName
     * @param tag
     */
