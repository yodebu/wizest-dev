...
public class AtTime implements Serializable {
	 ...
	 *      {일} : 1&tilde;31
	 *      {월} : 1&tilde;12
	 *      {요일} : 0&tilde;7 (단 0과 7은 모두 '일요일'을 의미 한다. 즉, 일0 월1 화2 수3 목4 금5 토6 일7)
	 *      [총 실행 횟수] : 0&tilde; (단, * 일 경우 실행 수의 제한이 없다. (defualt))
	 *      [년] : 1970&tilde;2200 , 생략가능 생략할 경우 매년 적용된다.
	...
	private void init(String crontab) {
		...
		try {
			...
			weekdays = parseScheduledTable(0, 7, weekday);
			...
			throw new IllegalArgumentException("invalid arguments:mins hours days months weekdays matchTimes years timezone :StackTrace=" + StackTrace.trace(t));
		}
	}
	 ...
	 * 
	 * match()가 true일 경우 match times 가 증가하므로 한 번 true 난 이후 설정 확인을 위해 계속 이 메소드를
	 * 콜하면 안된다. (실행 횟수의 제한에 영향을 준다.)
	 * 
	 * @return
	 */
	protected boolean match() {
		...
		int month = cal.get(Calendar.MONTH) + 1;
		int year = cal.get(Calendar.YEAR);
		int weekday = cal.get(Calendar.DAY_OF_WEEK) - 1; // 0 부터 시작하게
		int weekday2 = (weekday == 0) ? 7 : 0; // 0 과 7은 동일 -> AtTime 생성시 둘다
												// 허용했으므로 두번
		// 검색 ㅡ.ㅡ;;

		if (Arrays.binarySearch(mins, min) >= 0 && Arrays.binarySearch(hours, hour) >= 0
				&& ((Arrays.binarySearch(days, day) >= 0 || (Arrays.binarySearch(weekdays, weekday) >= 0 || Arrays.binarySearch(weekdays, weekday2) >= 0)) && Arrays.binarySearch(months, month) >= 0 && Arrays.binarySearch(years, year) >= 0)) {
	...
	public String toString() {
		...
		// weekdays
		buff.append("\tweekdays:");
		buff.append(getIntArrayString(weekdays));
		buff.append("\n");
	...
	/*
	 * public static void main(String[] args) { // AtTime at = new
	 * AtTime("20 4 1,15 1-12/2 6 * * GMT+9"); AtTime at = new
	 * AtTime("* 16 * 2 2 * * GMT+9"); // AtTime at = new
	 * AtTime(Calendar.getInstance()); System.out.println(at);
	 * System.out.println(at.match()); //
	 * System.out.println(System.currentTimeMillis()); //
	 * System.out.println(Calendar
	 * .getInstance(TimeZone.getTimeZone("GMT-1")).getTimeInMillis()); }
	 */
}
